...
void sauvegarderjeu(TAB joueur, TAB ia, char *nom, Player p)
{
    FILE *f=NULL;
    ...
    if(f!=NULL)
    {
        ...
        fprintf(f,"%d",p.nbtir);//sauvegarde du nombre de tir du joueur
    }
    else printf("erreur");
    fclose(f);
}
...
{
    FILE* fichier =NULL;
    fichier=fopen("SAUVEGARDEPARTIE.txt","r");//ouverture en lecture
    if(fichier!=NULL)
    {
        for(int i=0;i<9;i++)//copie valeur joueur
            for(int j=0;j<9;j++)
                chainejoueur[i][j].valeur=(int)fgetc(fichier)-48;//on afit -48 par rapport au code ASCII
        for(int k=0;k<9;k++)//copie etat joueur
            for(int l=0;l<9;l++)
                chainejoueur[k][l].etat=(int)fgetc(fichier)-48;
        for(int m=0;m<9;m++)//copie valeur ia
            for(int n=0;n<9;n++)
                chaineia[m][n].valeur=(int)fgetc(fichier)-48;
        for(int o=0;o<9;o++)//copie etat ia
            for(int p=0;p<9;p++)
                chaineia[o][p].etat=(int)fgetc(fichier)-48;
        p.nbtir=(int)fgetc(fichier)-48;
    }
    fclose(fichier);
}
